...
var lifecycleRe = /^(enter|update|merge|exit)(:transition)(first:)?$/;
...
Layer.prototype.on = function(eventName, handler, options) {
	...
	// TODO : Rework reg to check events
	// d3cAssert(
...
Layer.prototype.draw = function(data) {
	...
	for (var i = 0, l = events.length; i < l; ++i) {
		...
		if(!this.drawed && this._handlers['first:'+eventName]) {
			handlers = this._handlers['first:'+eventName];
		} else handlers = this._handlers[eventName];
		...
		if(!this.drawed && this._handlers['first:'+eventName+':transition']) {
			handlers = this._handlers['first:'+eventName+':transition'];
		} else handlers = this._handlers[eventName+':transition'];
	...
	this.drawed = true; // Used for first render logic
};
